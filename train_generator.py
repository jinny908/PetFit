...
def get_opt():
    ...
    # mps_device availability
    parser.add_argument('--mps_device',default=False, help='mps_device or cpu')
    ...
    if len(opt.gpu_ids) > 0:
        torch.mps_device.set_device(opt.gpu_ids[0])
...
def train(opt, train_loader, test_loader, test_vis_loader, board, tocg, generator, discriminator, model):
    ...
    # criterion
    if opt.fp16:
        criterionGAN = GANLoss('hinge', tensor=torch.mps_device.HalfTensor)
    else:
        criterionGAN = GANLoss('hinge', tensor=torch.mps_device.FloatTensor)
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step + opt.decay_step)):
        ...
        # input
        agnostic = inputs['agnostic'].mps_device()
        parse_GT = inputs['parse'].mps_device()
        pose = inputs['densepose'].mps_device()
        parse_cloth = inputs['parse_cloth'].mps_device()
        parse_agnostic = inputs['parse_agnostic'].mps_device()
        pcm = inputs['pcm'].mps_device()
        cm = inputs['cloth_mask']['paired'].mps_device()
        c_paired = inputs['cloth']['paired'].mps_device()
        
        # target
        im = inputs['image'].mps_device()
        ...
        with torch.no_grad():
            if not opt.GT:
                ...
                warped_cm_onehot = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(np.float)).mps_device()
            ...
            old_parse = torch.FloatTensor(fake_parse.size(0), 13, opt.fine_height, opt.fine_width).zero_().mps_device()
            ...
            parse = torch.FloatTensor(fake_parse.size(0), 7, opt.fine_height, opt.fine_width).zero_().mps_device()
        ...
        if not opt.no_ganFeat_loss:
            num_D = len(pred_fake)
            GAN_Feat_loss = torch.mps_device.FloatTensor(len(opt.gpu_ids)).zero_()
        ...
        if (step + 1) % opt.tensorboard_count == 0:
            ...
            parse_agnostic = inputs['parse_agnostic'].mps_device()
            pcm = inputs['pcm'].mps_device()
            cm = inputs['cloth_mask']['unpaired'].mps_device()
            c_paired = inputs['cloth']['unpaired'].mps_device()
...
def main():
    ...
    if len(opt.gpu_ids) > 0:
        assert(torch.mps_device.is_available())
        generator.mps_device()
