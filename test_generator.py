...
def test(opt, test_loader, tocg, generator):
    ...
    with torch.no_grad():
        for inputs in test_loader.data_loader:
            ...
            if opt.mps_device :
                pose_map = inputs['pose'].mps_device()
                pre_clothes_mask = inputs['cloth_mask'][opt.datasetting].mps_device()
                label = inputs['parse']
                parse_agnostic = inputs['parse_agnostic']
                agnostic = inputs['agnostic'].mps_device()
                clothes = inputs['cloth'][opt.datasetting].mps_device() # target cloth
                densepose = inputs['densepose'].mps_device()
                im = inputs['image']
                input_label, input_parse_agnostic = label.mps_device(), parse_agnostic.mps_device()
                pre_clothes_mask = torch.FloatTensor((pre_clothes_mask.detach().cpu().numpy() > 0.5).astype(float)).mps_device()
            else :
            ...
            if opt.mps_device :
                warped_cm_onehot = torch.FloatTensor((warped_clothmask_paired.detach().cpu().numpy() > 0.5).astype(float)).mps_device()
            else :
