...
def Evaluation(opt, pred_list, gt_list):
    ...
    model.eval()
    inception_model = inception_v3(pretrained=True, transform_input=False).type(torch.mps_device.FloatTensor)
    ...
    with torch.no_grad():
        ...
        for i, img_pred in enumerate(pred_list):
            ...
            # Calculate LPIPS
            gt_img_LPIPS = T2(gt_img).unsqueeze(0).mps_device()
            pred_img_LPIPS = T2(pred_img).unsqueeze(0).mps_device()
            ...
            pred_img_IS = T3(pred_img).unsqueeze(0).mps_device()
            preds[i] = F.softmax(inception_model(pred_img_IS)).data.cpu().numpy()

            gt_img_MSE = T1(gt_img).unsqueeze(0).mps_device()
            pred_img_MSE = T1(pred_img).unsqueeze(0).mps_device()
    ...
    f.write(f"IS_mean : {IS_mean} / ISq_std : {IS_std}\n")
