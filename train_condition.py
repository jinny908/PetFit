...
def train(opt, train_loader, test_loader, val_loader, board, tocg, D):
    ...
    if opt.fp16:
        criterionGAN = GANLoss(use_lsgan=True, tensor=torch.mps_device.HalfTensor)
    else :
        criterionGAN = GANLoss(use_lsgan=True, tensor=torch.mps_device.FloatTensor if opt.gpu_ids else torch.Tensor)
    ...
    for step in tqdm(range(opt.load_step, opt.keep_step)):
        ...
        cm_paired = inputs['cloth_mask']['paired'].mps_device()
        cm_paired = torch.FloatTensor((cm_paired.detach().cpu().numpy() > 0.5).astype(np.float)).mps_device()
        # input2
        parse_agnostic = inputs['parse_agnostic'].mps_device()
        densepose = inputs['densepose'].mps_device()
        openpose = inputs['pose'].mps_device()
        # GT
        label_onehot = inputs['parse_onehot'].mps_device()  # CE
        label = inputs['parse'].mps_device()  # GAN loss
        parse_cloth_mask = inputs['pcm'].mps_device()  # L1
        im_c = inputs['parse_cloth'].mps_device()  # VGG
        # visualization
        im = inputs['image']
        ...
        if (step + 1) % opt.tensorboard_count == 0:
            ...
            if not opt.no_test_visualize:
                ...
                # input1
                c_paired = inputs['cloth'][opt.test_datasetting].mps_device()
                cm_paired = inputs['cloth_mask'][opt.test_datasetting].mps_device()
                cm_paired = torch.FloatTensor((cm_paired.detach().cpu().numpy() > 0.5).astype(np.float)).mps_device()
                # input2
                parse_agnostic = inputs['parse_agnostic'].mps_device()
                densepose = inputs['densepose'].mps_device()
...
def main():
    ...
    os.environ["mps_device_VISIBLE_DEVICES"] = opt.gpu_ids
